...
def query_all_forecasts(
    ...
    """Query forecast rows across all regions for a single snapshot in a single batch.

    Avoids executing 22 individual SQL queries by fetching all available region rows
    for the specified or latest snapshot in one operation.
    ...
    Returns:
        pd.DataFrame: DataFrame with columns [region, forecast_start, forecast_end, min_temp, max_temp, fetched_at].
                      Sorted by region, forecast_start, forecast_end.
    ...
    # Construct parameterized SQL
    sql = BASE_QUERY_ALL_FORECASTS_SQL
    params: List[Any] = [dataset_id.strip(), snapshot_iso_utc]
    ...
    sql += " ORDER BY region ASC, forecast_start ASC, forecast_end ASC;"
    ...
    try:
        ...
        raise CwaQueryError(f"Failed to query all forecasts from '{db_path}': {exc}") from exc
    finally:
        conn.close()
